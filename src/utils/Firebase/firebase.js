...
class Firebase {
          ...
          .then(deploymentQuery => {
            ...
            for (var i = 0; i < deploymentIds.length; i++) {
              ...
              deploymentQuery[i]
                ...
                .catch(error => console.log("Error gettng all deployments: ", error));

              deployments.push(deployment);
            }
            return deployments;
          })
          .catch(error => console.log("Error getting all deployment refs: ", error));
      })
  ...
  setCustomLesson = (
    ...
    let currentAssignedDeploymentIds = customLessonRef.get("deploymentAccountIds");
    ...
    for (let deploymentAccountId of currentAssignedDeploymentIds) {
      if (!(deploymentAccountId in deploymentAccountIds)) {
        let deploymentRef = this.db.doc(`deployment_account/${deploymentAccountId}/`);
        batch.update(deploymentRef, {
    ...
    return batch
      .commit()
      .catch(error => console.log("Pushing custom lessons to deployments failed: ", error));
